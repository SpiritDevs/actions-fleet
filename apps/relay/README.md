...
The hook's execution SHA is stored separately from the REST head SHA: PR workflows can execute a synthetic merge commit, while PR-target workflows execute trusted base code. The relay verifies a PR merge's second parent against the approved head. For PR-target, it verifies the current base SHA or an ancestor on its history through GitHub's compare API. Unknown associations fail closed. An admitted job's returned `headSha` is the verified execution SHA; dashboard job metadata retains GitHub's canonical head SHA.
...
Console records expire thirty days after durable receipt; job/control metadata expires after ninety days. The default total accepted log payload bound is 256 MiB, with 32 MiB per job (`MAX_LOG_BYTES`, `MAX_JOB_LOG_BYTES`). SQL indexes and storage overhead are additional provider usage. Reaching a limit returns HTTP 507 without acknowledging new data: the host must preserve its spool and show the failure. This implementation uses Durable Object SQLite only; it does not claim an R2 archive or infinite storage.
